...
sys.path.append('../')
import constants
from utils import (convert_pixal_distance_to_meters, convert_meters_to_pixels_distance, get_foot_position, get_closest_keypoint_index, get_height_of_bbox, measure_xy_distance, get_center_of_bbox, measure_distance)
import numpy as np
...
class Mini_Court():
    ...
    def convert_meters_pixels(self, meter):
        return convert_meters_to_pixels_distance(meter, constants.DOUBLE_LINE_WIDTH, self.court_drawing_width)
    ...
    def set_court_drawing_key_points(self):
        ...
        # point 4
        drawing_key_points[8] = self.court_start_x + self.convert_meters_pixels(constants.DOUBLE_ALLY_DIFFERENCE)
        ...
        # point 6
        drawing_key_points[12] = drawing_key_points[2] - self.convert_meters_pixels(constants.DOUBLE_ALLY_DIFFERENCE)
        drawing_key_points[13] = drawing_key_points[3]

        # #point 7
        drawing_key_points[14] = drawing_key_points[6] - self.convert_meters_pixels(constants.DOUBLE_ALLY_DIFFERENCE)
...
# coordinate for player and ball of mini court
    def convert_bounding_box_to_mini_court_coordinate(self, player_boxes, ball_boxes, orginal_court_key_points):
        player_heights = {
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            for player_id, bbox in player_bbox.items():
                ...
                #get closet keypoint in pixel
                closest_key_point_index = get_closest_keypoint_index(foot_position, orginal_court_key_points, [0,2,12,13])
                closest_key_point = (orginal_court_key_points[closest_key_point_index * 2],
                                     orginal_court_key_points[closest_key_point_index * 2 + 1])
                ...
                #     closest_key_point_index = get_closest_keypoint_index(ball_position, orginal_court_key_points,
                #                                                          [0, 2, 12, 13])
                #     closest_key_point = (orginal_court_key_points[closest_key_point_index * 2],
                #                          orginal_court_key_points[closest_key_point_index * 2 + 1])
                #
    ...
    def get_mini_court_coordinates(self,
        ...
        distance_from_keypoint_x_meters = convert_pixal_distance_to_meters(distance_from_keypoint_x_pixels, player_height_in_meters, player_height_in_pixels)
        distance_from_keypoint_y_meters = convert_pixal_distance_to_meters(distance_from_keypoint_y_pixels, player_height_in_meters, player_height_in_pixels)
